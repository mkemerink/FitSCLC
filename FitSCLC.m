function varargout = FitSCLC(varargin)
% FITSCLC MATLAB code for FitSCLC.fig
%      FITSCLC, by itself, creates a new FITSCLC or raises the existing
%      singleton*.
%
%      H = FITSCLC returns the handle to a new FITSCLC or the handle to
%      the existing singleton*.
%
%      FITSCLC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FITSCLC.M with the given input arguments.
%
%      FITSCLC('Property','Value',...) creates a new FITSCLC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FitSCLC_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FitSCLC_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FitSCLC

% Last Modified by GUIDE v2.5 15-May-2018 13:14:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @FitSCLC_OpeningFcn, ...
    'gui_OutputFcn',  @FitSCLC_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before FitSCLC is made visible.
function FitSCLC_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FitSCLC (see VARARGIN)

% Choose default command line output for FitSCLC
...
handles.In.DDpar = [1.0e-9;... [m] dx - mesh size (0.1...1e-9)
    ...
    200;...  [1] In.MaxIt - max nr. of iterations in self-consistent loop (200)
...
% UIWAIT makes FitSCLC wait for user response (see UIRESUME)
% uiwait(handles.the_GUI);


% --- Outputs from this function are returned to the command line.
function varargout = FitSCLC_OutputFcn(hObject, eventdata, handles)
...
handles.muModel.Value = 1; %reset muModel to 1st (default) element to prevent errors
handles.In.muModel = 1;
...
if handles.In.FitModel==1 && handles.DATA.NrTemp>1 %plot gamma & mu0 vs T if applicable
    Make_gammaPlot(handles)
    Make_mu0Plot(handles)
else %otherwise remove old figures
    cla(handles.gammaPlot)
    cla(handles.mu0Plot)
end
